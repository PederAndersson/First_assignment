...
#include <iostream>
#include <cmath>
Statistics Statistics::calculateStatistics(const std::vector<double> &store_value){

    Statistics stats;

    stats.number_entries = static_cast<int>(store_value.size());
    stats.sum = Calculations::vectorSum(store_value);
    stats.mean = stats.sum/static_cast<double>(store_value.size());
    stats.value_min = Calculations::minValue(store_value);
    stats.value_max = Calculations::maxValue(store_value);
    stats.variance = Calculations::varianceValue(store_value);
    stats.standard_dev = sqrt(stats.variance);
    return stats;
}

void Statistics::printStatistics(const Statistics& stats){
